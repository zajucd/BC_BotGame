...
IsEasyMode = false;
//这里是快速结局模式的开关，设置成true电子拘束具就不会被其他拘束具替换
IsQuickEnding = false;
	...
	this.ForceWearEquip = function (wearer) {
		...
		if(wearer.slots[this.slot] !== null){
			...
			//该槽位以存在其他装备时处理
				...
				//若在快速结局模式下且将移除电子装备，则返回
				if(IsQuickEnding === true && wearer.slots[this.slot].id >= 100){
					ServerSend("ChatRoomChat", { Content: "*但是电子装备无法被移除", Type: "Emote", Target: wearer.character.MemberNumber} );
					return;
				}
		...
		for(var i = 0; i< this.useSlot.length; i++){
			if (wearer.slots[this.useSlot[i]] !== null && typeof wearer.slots[this.useSlot[i]] !== "number"){
				if(IsQuickEnding === true && wearer.slots[this.useSlot[i]].id >= 100){
					ServerSend("ChatRoomChat", { Content: "*但是电子装备无法被移除", Type: "Emote", Target: wearer.character.MemberNumber} );
					return;
				}
			}
...
function QuestStruct() {
	...
	this.QuestSuccess = function (player, coin) {
		if(remainedCoinOfEachFloor[parseInt(player.location.replace(/[^\d]/g, " "))] >= coin){
			...
			ServerSend("ChatRoomChat", { Content: "*当前层无金币剩余", Type: "Emote", Target: player.character.MemberNumber} );
		}
		else{
