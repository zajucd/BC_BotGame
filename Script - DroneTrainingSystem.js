...
const MsgCmds = {
    HeartBeatPack: {
        Command: (sender, param) => {
            if (param) {
                SendDTSMsg(sender, new MsgInfo("HeartBeatPack", false));
            }
        }
    },
    SetStatus: {
...
function InstallHook(funcName, context, beforeFn, afterFn) {
    // 确定上下文对象
...
function Init() {
    ...
    InstallHook("ChatRoomMapViewMove", null, ChatRoomMapViewMoveBefore, null)
    ...
    if ((data != null) && (typeof data === "object") && (data.Content != null) && (typeof data.Content === "string") && (data.Content != "") && (data.Sender != null) && (typeof data.Sender === "number")) {
        ...
        if (SenderCharacter != null) {
            ...
            if (data.Content == "DTS" && data.Type == 'Hidden') {
                ...
                //高潮充电
                if (data.Content.startsWith("Orgasm") && data.Dictionary.length >= 1 && typeof data.Dictionary[0].SourceCharacter == 'number' && CheckPlayerDroneInfoExistAndIsDrone()) {
                    if (data.Dictionary[0].SourceCharacter == Player.MemberNumber) {
                ...
                if (charaterInstalledScript[SenderCharacter.MemberNumber]) {
                    delete charaterInstalledScript[SenderCharacter.MemberNumber];
                }
            }
...
function PlayerEnterRoom() {
    ...
    charaterInstalledScript = {};
    ShowPlayerEnterHelp();
}

function DoHiddenMessage(ChatRoomCharacter, msg, dict) {
    if (!charaterInstalledScript[ChatRoomCharacter.MemberNumber]) {
        charaterInstalledScript[ChatRoomCharacter.MemberNumber] = true;
    }
 ...
 * @param {any} D
 */
function ChatRoomMapViewMoveBefore(D) {
...
function ServerShowBeepBefore(message, duration, options, title) {
    ...
    if (message.toLowerCase().startsWith("DTSBeep")) {
        var memberNumber = -1;
        memberNumber = options.memberNumber;
        var params = message.split(' ');
        if (params.length > 2) {
            msgInfo.DoBeepCmd(memberNumber, new MsgInfo(params[1], params.slice(2)));
        }
        else {
            msgInfo.DoBeepCmd(memberNumber, new MsgInfo(params[1], null));
        }
        return 0;
    }
}
...
function ShowPlayerEnterHelp() {
    ...
    SendDTSMsg(null, new MsgInfo("HeartBeatPack", true));
...
function ResponseRequestStatus(sender) {
    if (charaterInstalledScript[sender.MemberNumber]) {
        SendDTSMsg(sender, new MsgInfo("RecivedStatus", PlayerDroneInfo()));
    }
...
function ShowActionButtons(info = null) {
    ...
    if (info.MemberNumber == Player.MemberNumber) {
        if (info.isDrone) SendMessageToSelf(ShowStringsToSelf(0, info));
        else if (info.isOwner) SendMessageToSelf(ShowStringsToSelf(1, info));
        else SendMessageToSelf(ShowStringsToSelf(2, info));
    }
    //对其它人
    else {
        var pdi = PlayerDroneInfo();
        if (pdi.isDrone) {
            if (info.isDrone) SendMessageToSelf(ShowStringsToOther(0, info));
            else if (info.MemberNumber == pdi.ownerId) SendMessageToSelf(ShowStringsToOther(1, info));
            else SendMessageToSelf(ShowStringsToOther(2, info));
        }
        else if (pdi.isOwner) {
            if (info.isDrone) SendMessageToSelf(ShowStringsToOther(3, info));
            else SendMessageToSelf(ShowStringsToOther(5, info));
        }
        else {
            if (info.isDrone) SendMessageToSelf(ShowStringsToOther(4, info));
            else SendMessageToSelf(ShowStringsToOther(5, info));
        }
    }
}
...
显示任务进度:${styleButton("执行", ShowQuestProcess)}
...
发送任务求助:${styleButton("执行", SendQuestHelp, info)}
...
设置任务:${styleButton("执行", SetQuestToDrone, info)}
...
function DoFindTatget(target) {
    SendDTSMsg(target, new MsgInfo("RequestStatus", null));
}
//WIP
function ShowQuestProcess() {
    SendMessageToSelf("任务功能开发中，预计和训练设施地图同步上线");
}

//WIP
function SendQuestHelp(info) {
    SendMessageToSelf("任务功能开发中，预计和训练设施地图同步上线");
}

//WIP
function SetQuestToDrone(info) {
    SendMessageToSelf("任务功能开发中，预计和训练设施地图同步上线");
}
...
async function StartDrone() {
    ...
    await sleep(waitTime)
    ActivityTimerProgress(Player, 10);
    SendMessageToSelf(`总电源安装管已到达子宫口，开始向子宫内植入总电源`);
    await sleep(waitTime)
    ActivityTimerProgress(Player, 10);
    SendMessageToSelf(`总电源植入成功，开始向总电源内注入电池液`);
    await sleep(waitTime)
    ActivityTimerProgress(Player, 10);
...
function SetStatusSend(info, type, part, level) {
    if ((type == 1 && info.bodyStatusMax[bodyPartStrings[part]] < level)&& part != 3) {
        SendMessageToSelf("当前无人机该部位未接受改造手术，无法设置为指定状态");
        return;
    }
...
class MsgInfo {
    ...
    static DoBeepCmd(MemberNumber, msgInfo) {
        BeepCmds[msgInfo.type].Command(MemberNumber, msgInfo.param)
    }
...
function SendMessageToSelf(message, cantClear = false) {

    ChatRoomSendLocal(styleMessage(message, cantClear));
}
...
function styleMessage(message, cantClear = false) {
    var timestamp = new Date().getTime(); // 函数调用时的时间
    if (cantClear) timestamp = false
    const hiddenString = "styleMessage";

    return `<div data-timestamp="${timestamp}" data-style-message="${hiddenString}" style='
  background: #000000;
  padding: 8px;
